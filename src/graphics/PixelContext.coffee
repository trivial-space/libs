...
class tslibs.graphics.PixelContext
  ...
  drawImageAt: (pixelCtx, [x, y]) ->
    ...
    for i in [0..w]
      for j in [0..h]
        @setColorAt [x+i, y+j],
          tslibs.graphics.mixColors @getColorAt([x+i, y+j]), pixelCtx.getColorAt [i, j]
  ...
  mixinColor: (color) ->
    ...
    for x in [0..w]
      for y in [0..h]
        @setColorAt [x, y],
          tslibs.graphics.mixColors @getColorAt([x, y]), color



class tslibs.graphics.ClosedPixelContext extends tslibs.graphics.PixelContext
...
class tslibs.graphics.RepeatedPixelContext extends tslibs.graphics.PixelContext
...
tslibs.graphics.createCtxFromImg = (img, ContextClass) ->
