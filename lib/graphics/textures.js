...
export function drawTileNoiseTexture(ctx, startX, startY) {
    const width = ctx.canvas.width, height = ctx.canvas.height, noise1 = tileNoise(width, height, startX, startY), noise2 = tileNoise(width, height, startX * 2.1, startY * 2.1), noise3 = tileNoise(width, height, startX * 4.2, startY * 4.2), noise4 = tileNoise(width, height, startX * 8.3, startY * 8.3), imgData = ctx.getImageData(0, 0, width, height);
