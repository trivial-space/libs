...
export function makePerspective({ val }, canvasSize, fovy, near, far) {
    fovy = fovy || val(Math.PI * 0.5);
    near = near || val(0.1);
    far = far || val(1000);
    const aspect = val(1)
        .react([canvasSize.HOT], size => size.width / size.height);
...
export function makeFirstPersonView({ val }, position, yaw, pitch) {
    ...
    const rotationQuat = val(quat.create())
        .react([yawQuat.HOT, pitchQuat.HOT], quat.multiply);
